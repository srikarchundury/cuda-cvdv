...
###############################################################################
# App4 same-final-circuit campaign runner for Perlmutter GPU
#
# Same as run.sh but all backends use the shared bosonic-qiskit frontend
# circuit, so timing differences reflect only the backend, not circuit
# construction. Calls same_final_circuits_run.py (this directory).
#
# Output structure:
#   raw_data/perlmutter_gpu/app4_same_final/runs/<campaign>/
#   app4_same_final__<method>__<tag>__<cfg_label>.out
#
...
BENCHMARK_NAME="app4_same_final"
...
[[ -f "$SCRIPT_DIR/same_final_circuits_run.py" ]] || die "same_final_circuits_run.py not found"
...
# Build manifest: all methods share the same runner script
...
for dv in "${DV_QUBITS[@]}"; do
	for cv in "${CV_QUBITS[@]}"; do
		...
		for method in "${APP4_METHODS[@]}"; do
			cmd="${PYTHON_BIN} same_final_circuits_run.py --method ${method} --dv-qubits ${dv} --cv-qubits ${cv} --lam ${LAM} --runs ${ITERS} --warmup ${WARMUP}"
...
echo "Iters      : ${ITERS}  Warmup: ${WARMUP}  Lambda: ${LAM}"
